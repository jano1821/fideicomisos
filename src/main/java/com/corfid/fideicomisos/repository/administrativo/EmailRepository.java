...
public interface EmailRepository extends JpaRepository<Email, Serializable> {
    ...
    @Query(value = "select d from Email d where d.persona.idPersona = :idPersona", countQuery = "select count(d) from Email d where d.persona.idPersona = :idPersona")
    public Page<Email> findEmailByIdPersona(@Param("idPersona") Integer idPersona, Pageable pageable);
