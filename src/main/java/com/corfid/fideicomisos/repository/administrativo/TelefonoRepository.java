...
public interface TelefonoRepository extends JpaRepository<Telefono, Serializable> {
    @Query(value = "select d from Telefono d where d.persona.idPersona = :idPersona",
      countQuery = "select count(d) from Telefono d where d.persona.idPersona = :idPersona")
    public Page<Telefono> findTelefonoByIdPersona(@Param("idPersona") Integer idPersona, 
                                                  Pageable pageable);
